...
async function getOutputRowsForXactn(xactn, walletId) {
  ...
  let stxHeadersIn = await convertTransfersToRowHeader(
    nftTransfersOut.length > 0,
    stxTransfersIn,
    false
  );
  let ftHeadersIn = await convertTransfersToRowHeader(
    nftTransfersOut.length > 0,
    ftTransfersIn,
    false
  );
  let nftHeadersIn = await convertTransfersToRowHeader(false, nftTransfersIn, true);
  let stxHeadersOut = await convertTransfersToRowHeader(
    nftTransfersIn.length > 0,
    stxTransfersOut,
    false
  );
  let ftHeadersOut = await convertTransfersToRowHeader(
    nftTransfersIn.length > 0,
    ftTransfersOut,
    false
  );
...
async function getOutputArrayRow(xactn, xactnFee, transferIn, transferOut) {
  ...
  let inSymbol =
    transferIn === undefined ? '' : transferIn.symbol === undefined ? '' : transferIn.symbol;
  let outSymbol =
    transferOut === undefined ? '' : transferOut.symbol === undefined ? '' : transferOut.symbol;
  let inAmountRaw =
    transferIn === undefined ? 0 : transferIn.rawAmount === undefined ? 1 : transferIn.rawAmount;
  let outAmountRaw =
    transferOut === undefined ? 0 : transferOut.rawAmount === undefined ? 1 : transferOut.rawAmount;
  let isNftIn =
    transferIn === undefined ? '' : transferIn.isNft === undefined ? '' : transferIn.isNft;
  let isNftOut =
    transferOut === undefined ? '' : transferOut.isNft === undefined ? '' : transferOut.isNft;
  ...
  let memo = xactn?.tx?.token_transfer?.memo
    ? hexToAscii(xactn.tx.token_transfer.memo.substring(2)).trim()
    : '';
  if (memo.length < 1) {
    memo = '';
  }
  ...
  if (symbol !== '') {
    ...
    if (coinPriceObject !== undefined) {
      ...
      if (matchingPrice.length > 0) {
        ...
        if (
          formattedPrice === 'N/A' &&
          new Date(priceDate).getTime() >= new Date(2022, 0, 1).getTime()
        ) {
          if (symbol === 'EUR') {
