...
export default async function getBlockInfo() {
  ...
  var miaCyclePercentComplete = Math.floor(
    ((currentBlock - miaCycleStartingBlock) * 100) / (miaCycleEndBlock - miaCycleStartingBlock)
  );
  ...
  var nycCyclePercentComplete = Math.floor(
    ((currentBlock - nycCycleStartingBlock) * 100) / (nycCycleEndBlock - nycCycleStartingBlock)
  );
...
export async function getTrackedBlocks() {
  ...
  var futureBlocks = blocksJson.flatMap((x) =>
    isPast(x.blockheight, currentBlock)
      ? {
          ...x,
          when: whenis(x.blockheight, currentBlock),
          past: isPast(x.blockheight, currentBlock),
        }
      : []
  );
  ...
  var pastBlocks = blocksJson.flatMap((x) =>
    !isPast(x.blockheight, currentBlock)
      ? {
          ...x,
          when: whenis(x.blockheight, currentBlock),
