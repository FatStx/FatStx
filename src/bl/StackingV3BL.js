...
import {
  filterStackingResultsForOutput,
  formatStackingResultsForOutput,
  createOutputRow,
} from './StackingSharedBL';
...
export default async function getStackingReportArrayV3(walletId, symbol, latestBitcoinBlock) {
  ...
  if (userId === null) {
    outputArray.push({
      message:
        'Unexpected error trying to obtain City Coins data. Please try again in a few minutes.',
    });
    return outputArray;
  } else if (userId === 0) {
  ...
  if (latestBitcoinBlock === null) {
    outputArray.push({
      message:
        'Unexpected error trying to obtain latest bitcoin block. Please refresh browser and try again in several minutes',
    });
    return outputArray;
  }
  ...
  if (errorFlag) {
    outputArray.push({
      message: 'Unexpected error checking stacking cycle data. Please try again in a few minutes.',
    });
    return outputArray;
  }
