...
function LinearProgressWithLabel(props) {
    return (
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        ...
        <Box sx={{ minWidth: 35 }}>
          <Typography sx={{fontSize: 12, fontWeight: 300}} variant="body1" color="textSecondary">{`${Math.round(
            props.value,
          )}%`}</Typography>
        </Box>
      </Box>
...
export default function WenblokCycles() {
    ...
    useEffect(() => {
      (async () => {
        ...
        setNycDaysLeftInCycle( Math.floor((blockInfo.nycCycleEndBlock - blockInfo.currentBlock) * 10 / 60 / 24) )
    ...
    return(
        <Grid container spacing={3}>
            <Grid item xs={12} sm={3} md={3}>
                ...
                <CardContent>
                    <Typography align="center" sx={{ fontSize: 40, fontWeight: 100}} variant="h5" color="#344767" component="div">
                    {currentBlock}
                    </Typography>
            ...
            <Grid item xs={12} sm={3} md={3}>
                <Card sx={{ minWidth: 275, height:"100%" }}>
                <CardContent>
                    <Typography align="center" sx={{ fontSize: 20, fontWeight: 500, pt:2, pb:1 }} variant="h5" color="#344767" component="div">
